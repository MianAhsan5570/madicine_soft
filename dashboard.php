...
while ($fetchOrder = mysqli_fetch_assoc($salesGet)) {
    ...
    $sql = "SELECT * FROM order_item 
            WHERE order_id = '{$fetchOrder['order_id']}' 
            AND order_item_status = 1";
    $query = $dbc->query($sql);
    ...
    while ($result = $query->fetch_assoc()) {
        ...
        // get purchase rate
        $sql_item = "SELECT rate FROM purchase_item 
                     WHERE product_id = '$product_id' 
                     ORDER BY purchase_item_id DESC LIMIT 1";
        $query_item = $dbc->query($sql_item);
        $result_item = $query_item->fetch_assoc();

        $product_purchase = $result_item['rate'] ?? 0;
...
# -------------------------------
# SALE RETURNS PROFIT REVERSAL
# -------------------------------

$returnQuery = mysqli_query($dbc, "SELECT * FROM orders_return WHERE 1=1 $date_select");
$total_return_loss = 0;

while ($returnOrder = mysqli_fetch_assoc($returnQuery)) {

    $return_id = $returnOrder['order_id']; // ✅ correct PK

    // ✅ correct relation
    $return_items_query = $dbc->query("
        SELECT * FROM order_return_item 
        WHERE order_id = '$return_id'
    ");

    while ($return_item = $return_items_query->fetch_assoc()) {

        $product_id = $return_item['product_id'];
        $return_quantity = $return_item['quantity'];
        $return_rate = $return_item['rate'];

        // get purchase rate
        $purchase_query = $dbc->query("
            SELECT rate FROM purchase_item 
            WHERE product_id = '$product_id' 
            ORDER BY purchase_item_id DESC LIMIT 1
        ");
        $purchase_item = $purchase_query->fetch_assoc();
        $purchase_rate = $purchase_item['rate'] ?? 0;

        // profit that must be reversed
        $return_profit_loss = ($return_rate - $purchase_rate) * $return_quantity;

        $total_return_loss += $return_profit_loss;
    }
}
...
# FINAL PROFIT
# -------------------------------
$total_profit = $total_profit - $total_return_loss;
