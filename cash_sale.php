          ...
          <form action="php_action/custom_action.php" method="POST" id="sale_order_fm">
            ...
            <div class="form-group row mt-3 mb-3">
                ...
                <select class="form-control searchableSelect product_dropdown" id="get_product_name" name="product_id">
                  ...
                  while ($row = mysqli_fetch_array($result)) {
                    ...
                    <option data-price="<?= $row["current_rate"] ?>" <?= empty($r['product_id']) ? "" : "selected" ?>
                      ...
                      <!-- <?= ucwords(@$getBrand["brand_name"]) ?>(<?= @$getCat["categories_name"] ?>) -->
                    </option>
              ...
              <div class="col-6 col-sm-2 col-md-1">
                ...
                <input type="number" data-max="" class="form-control" placeholder="Qty" id="get_product_quantity"
                  value="1" min="1" name="quantity">
              </div>

              <div class="col-6 col-sm-2 col-md-1">
                <label>Bonus Qty</label>
                <input type="number" class="form-control" placeholder="Bonus" id="get_bonus_qty" value="0" min="0">
              </div>
                      ...
                      while ($r = mysqli_fetch_assoc($q)) {
                        ...
                        <tr id="product_idN_<?= $r['product_id'] ?>_<?= $r['batch_id'] ?? '0' ?>">
                          <input type="hidden" data-price="<?= $r['rate'] ?>" data-quantity="<?= $r['quantity'] ?>"
                            data-discount="<?= $item_disc ?>" data-bonus="<?= $item_bonus ?>"
                            data-tax="<?= (float) ($r['tax'] ?? 0) ?>"
                            id="product_ids_<?= $r['product_id'] ?>_<?= $r['batch_id'] ?? '0' ?>" class="product_ids"
                            name="product_ids[]" value="<?= $r['product_id'] ?>">
                          <input type="hidden" id="product_quantites_<?= $r['product_id'] ?>_<?= $r['batch_id'] ?? '0' ?>"
                            name="product_quantites[]" value="<?= $r['quantity'] ?>">
                          <input type="hidden" id="product_totalrate_<?= $r['product_id'] ?>_<?= $r['batch_id'] ?? '0' ?>"
                            name="product_totalrates[]" value="<?= $item_total ?>">
                  ...
                  <tfoot>
                    <tr>
                      ...
                      <td class="table-bordered" id="product_total_amount">
                        <?= number_format((@$fetchOrder['total_amount'] ?? 0) + (@$fetchOrder['tax'] ?? 0), 2) ?></td>
                        ...
                        <div class="row">
                          <div class="col-sm-6 pr-0">
                            <input onkeyup="getOrderTotal()" type="number" id="ordered_discount"
                              ...
                              max="100" step="0.01" name="ordered_discount">
                          </div>
